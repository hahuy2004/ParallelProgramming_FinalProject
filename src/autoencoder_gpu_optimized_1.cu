...
AutoencoderGPUOptimized1::AutoencoderGPUOptimized1() {
    ...
    std::cout << "GPU Optimized v1 (Shared Memory) weights initialized" << std::endl;
}
...
void AutoencoderGPUOptimized1::forward() {
    ...
    launch_conv2d_shared_forward(d_input, d_conv1_out, d_conv1_weight, d_conv1_bias,
                         INPUT_H, INPUT_W, INPUT_C, CONV1_FILTERS, INPUT_H, INPUT_W, 3, 1, 1);
    ...
    // kernel_size=3, stride=1, padding=1
    launch_conv2d_shared_forward(d_pool2_out, d_conv3_out, d_conv3_weight, d_conv3_bias,
                         LATENT_H, LATENT_W, LATENT_C, LATENT_C, LATENT_H, LATENT_W, 3, 1, 1);
    ...
    // kernel_size=3, stride=1, padding=1
    launch_conv2d_shared_forward(d_up1_out, d_conv4_out, d_conv4_weight, d_conv4_bias,
                         INPUT_H/2, INPUT_W/2, LATENT_C, CONV1_FILTERS, INPUT_H/2, INPUT_W/2, 3, 1, 1);
    ...
    // kernel_size=3, stride=1, padding=1
    launch_conv2d_shared_forward(d_up2_out, d_conv5_out, d_conv5_weight, d_conv5_bias,
                         INPUT_H, INPUT_W, CONV1_FILTERS, INPUT_C, INPUT_H, INPUT_W, 3, 1, 1);
}

void AutoencoderGPUOptimized1::backward() {
    // Zero out ALL gradient buffers to prevent accumulation across batches
    CUDA_CHECK(cudaMemset(d_conv1_weight_grad, 0, CONV1_FILTERS * INPUT_C * 3 * 3 * sizeof(float)));
    CUDA_CHECK(cudaMemset(d_conv1_bias_grad, 0, CONV1_FILTERS * sizeof(float)));
    CUDA_CHECK(cudaMemset(d_conv2_weight_grad, 0, CONV2_FILTERS * CONV1_FILTERS * 3 * 3 * sizeof(float)));
    CUDA_CHECK(cudaMemset(d_conv2_bias_grad, 0, CONV2_FILTERS * sizeof(float)));
    CUDA_CHECK(cudaMemset(d_conv3_weight_grad, 0, LATENT_C * LATENT_C * 3 * 3 * sizeof(float)));
    CUDA_CHECK(cudaMemset(d_conv3_bias_grad, 0, LATENT_C * sizeof(float)));
    CUDA_CHECK(cudaMemset(d_conv4_weight_grad, 0, CONV1_FILTERS * LATENT_C * 3 * 3 * sizeof(float)));
    CUDA_CHECK(cudaMemset(d_conv4_bias_grad, 0, CONV1_FILTERS * sizeof(float)));
    CUDA_CHECK(cudaMemset(d_conv5_weight_grad, 0, INPUT_C * CONV1_FILTERS * 3 * 3 * sizeof(float)));
    CUDA_CHECK(cudaMemset(d_conv5_bias_grad, 0, INPUT_C * sizeof(float)));
    
    CUDA_CHECK(cudaMemset(d_grad_up2, 0, CONV1_FILTERS * INPUT_H * INPUT_W * sizeof(float)));
    CUDA_CHECK(cudaMemset(d_grad_relu4, 0, CONV1_FILTERS * (INPUT_H/2) * (INPUT_W/2) * sizeof(float)));
    CUDA_CHECK(cudaMemset(d_grad_conv4, 0, CONV1_FILTERS * (INPUT_H/2) * (INPUT_W/2) * sizeof(float)));
    CUDA_CHECK(cudaMemset(d_grad_up1, 0, LATENT_C * (INPUT_H/2) * (INPUT_W/2) * sizeof(float)));
    CUDA_CHECK(cudaMemset(d_grad_relu3, 0, LATENT_C * LATENT_H * LATENT_W * sizeof(float)));
    CUDA_CHECK(cudaMemset(d_grad_conv3, 0, LATENT_C * LATENT_H * LATENT_W * sizeof(float)));
    CUDA_CHECK(cudaMemset(d_grad_pool2, 0, LATENT_C * (INPUT_H/2) * (INPUT_W/2) * sizeof(float)));
    CUDA_CHECK(cudaMemset(d_grad_relu2, 0, CONV2_FILTERS * (INPUT_H/2) * (INPUT_W/2) * sizeof(float)));
    CUDA_CHECK(cudaMemset(d_grad_conv2, 0, CONV2_FILTERS * (INPUT_H/2) * (INPUT_W/2) * sizeof(float)));
    CUDA_CHECK(cudaMemset(d_grad_pool1, 0, CONV1_FILTERS * INPUT_H * INPUT_W * sizeof(float)));
    CUDA_CHECK(cudaMemset(d_grad_relu1, 0, CONV1_FILTERS * INPUT_H * INPUT_W * sizeof(float)));
    CUDA_CHECK(cudaMemset(d_grad_conv1, 0, CONV1_FILTERS * INPUT_H * INPUT_W * sizeof(float)));
    
    // Conv5 backward - kernel_size=3, stride=1, padding=1
    launch_conv2d_shared_backward(d_grad_conv5, d_up2_out, d_conv5_weight, d_grad_up2,
                                   d_conv5_weight_grad, d_conv5_bias_grad,
                                   INPUT_H, INPUT_W, CONV1_FILTERS, INPUT_C, INPUT_H, INPUT_W, 3, 1, 1);
    ...
    launch_conv2d_shared_backward(d_grad_conv2, d_pool1_out, d_conv2_weight, d_grad_pool1,
                                   ...
                                   INPUT_H/2, INPUT_W/2, CONV1_FILTERS, CONV2_FILTERS, INPUT_H/2, INPUT_W/2, 3, 1, 1);
    ...
    // Note: We don't need d_grad_input for Conv1 as it's the first layer
    float* d_grad_input_dummy = nullptr;
    CUDA_CHECK(cudaMalloc(&d_grad_input_dummy, INPUT_C * INPUT_H * INPUT_W * sizeof(float)));
    launch_conv2d_shared_backward(d_grad_conv1, d_input, d_conv1_weight, d_grad_input_dummy,
                                   d_conv1_weight_grad, d_conv1_bias_grad,
                                   INPUT_H, INPUT_W, INPUT_C, CONV1_FILTERS, INPUT_H, INPUT_W, 3, 1, 1);
    cudaFree(d_grad_input_dummy);
}
...
bool AutoencoderGPUOptimized1::load_weights(const std::string& filepath) {
    ...
    auto load_tensor = [&](float* d_ptr, int expected_size) -> bool {
        int size;
...
void AutoencoderGPUOptimized1::extract_features(const float* input_chw, float* output_features) {
    ...
    // Conv2: (256, 16, 16) -> (128, 16, 16)
    // kernel_size=3, stride=1, padding=1
    launch_conv2d_shared_forward(d_pool1_out, d_conv2_out, d_conv2_weight, d_conv2_bias,
                         INPUT_H/2, INPUT_W/2, CONV1_FILTERS, CONV2_FILTERS, INPUT_H/2, INPUT_W/2, 3, 1, 1);
